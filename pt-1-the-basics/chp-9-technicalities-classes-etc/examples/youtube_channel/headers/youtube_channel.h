...
class YouTubeChannel {
  public:
    ...
    void set_owner(const std::string& new_owner) { owner_name = new_owner; }
    void set_about(const std::string new_about) { about = new_about; }

    // getters
    void get_name() { std::cout << name << '\n'; }
    void get_owner() { std::cout << owner_name << '\n'; }
    void get_about() { std::cout << name << ": "<< about << '\n'; }
...
class SingingYouTubeChannel:public YouTubeChannel {
  public:
    SingingYouTubeChannel(std::string name_, std::string owner_name_)
      :YouTubeChannel(name_, owner_name_) {}
};
